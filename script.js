const apiKey = 'fd264bffd234533a98c0391313e2552d'; // Mi clave API
...
async function showMovieDetails(movieId) {
    try {
        ...
        if (!response.ok) {
            throw new Error('Error en la solicitud de detalles');
        }
        const movie = await response.json();

        selectedMovieId = movie.id; // Guarda el ID de la película seleccionada
        
        // Remueve la clase 'hidden' del contenedor de detalles
